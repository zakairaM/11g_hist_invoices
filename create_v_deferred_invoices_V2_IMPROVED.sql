-- ============================================================================
-- View: teamblue.dwh.v_deferred_invoices (IMPROVED VERSION)
-- ============================================================================
...
-- - Uses ssas_f_invoices_accrued_v2 (better metrics than f_invoices_accrued)
-- - M_AMOUNT_EUR for period-level amounts (not invoice totals!)
-- - M_MRR_EOP_EUR for accurate MRR per period
-- - Fewer joins required (more FKs already in v2)
-- - More accurate comparison with silver.deferred_revenue
-- ============================================================================
-- Created: 2025-11-18
...
SELECT
    ...
    -- ========================================================================
    -- Invoice identifiers (need to get BK codes from f_invoices)
    -- ========================================================================
    ...
    -- ========================================================================
    -- IMPROVED: Period-level amounts from ssas_f_invoices_accrued_v2!
    -- ========================================================================
    ...
    -- ========================================================================
    -- Deferral dates and amounts - IMPROVED with actual period amounts!
    -- ========================================================================
    CAST(d_accrued.BK_DATE AS DATE) as planned_posting_date,
    CAST(d_accrued.BK_DATE AS DATE) as actual_posting_date,
    
    -- IMPROVED: Use actual period amounts from v2 (not invoice totals!)
    ...
    -- ========================================================================
    -- Additional DWH-specific fields for analysis (IMPROVED from V2)
    -- ========================================================================
    ...
    -- Customer and subscription movement flags
...
FROM 
    -- ========================================================================
    -- IMPROVED: Base table is now ssas_f_invoices_accrued_v2 (not f_invoices_accrued)
    -- This has better metrics and is optimized for BI/reporting
    -- ========================================================================
    teamblue.dwh.ssas_f_invoices_accrued_v2 sia
    
    -- ========================================================================
    -- Join to f_invoices for business keys and descriptions
    -- ========================================================================
    ...
    LEFT JOIN teamblue.dwh.d_date d_invoice
        ON sia.FK_DATE_INVOICE = d_invoice.PK_DATE
    ...
    LEFT JOIN teamblue.dwh.d_date d_accrued
        ON sia.FK_DATE_ACCUED = d_accrued.PK_DATE
    
    -- ========================================================================
    -- Customer dimensions
    -- ========================================================================
    LEFT JOIN teamblue.dwh.d_customers cust
        ON sia.FK_CUSTOMERS = cust.PK_CUSTOMERS
    
    -- ========================================================================
    -- Geography dimension
    -- ========================================================================
    LEFT JOIN teamblue.dwh.d_geography geo
        ON sia.FK_GEOGRAPHY_CUSTOMER = geo.PK_GEOGRAPHY
    
    -- ========================================================================
    -- Product dimensions
    -- ========================================================================
    LEFT JOIN teamblue.dwh.d_products prod
        ON sia.FK_PRODUCTS = prod.PK_PRODUCTS
    LEFT JOIN teamblue.dwh.d_product_segment prod_seg
        ON sia.FK_PRODUCT_SEGMENT = prod_seg.PK_PRODUCT_SEGMENT
    ...
    LEFT JOIN teamblue.dwh.d_currency curr
        ON sia.FK_CURRENCY = curr.PK_CURRENCY
    
    -- ========================================================================
    -- Invoice source dimension (from f_invoices)
    -- ========================================================================
...
WHERE
    ...
    -- ========================================================================
    -- Filter 2: Only RegularMRR type (improved flag from v2)
    -- ========================================================================
...
--
-- 1. BASE TABLE CHANGE:
--    - FROM: f_invoices + f_invoices_accrued (INNER JOIN)
--    - TO:   ssas_f_invoices_accrued_v2 (already has period-level data)
--
-- 2. BETTER AMOUNT FIELDS:
--    - OLD: Used fi.M_INVOICE_MRR (invoice-level total, not accurate per period)
--    - NEW: Uses sia.M_AMOUNT_EUR (actual period-level amount!)
--
-- 3. MORE ACCURATE METRICS:
--    - M_MRR_EOP_EUR: End of period MRR (accurate per period)
--    - M_QUANTITY: Period-level quantity
--    - M_AMOUNT_EUR: Period-level amount in EUR
--
-- 4. ADDITIONAL ANALYSIS FLAGS:
--    - Customer movement (new, churn)
--    - Revenue movements (upsell, downsell, cross-sell)
--    - Subscription movements (renewed, added, lost)
--
-- 5. SIMPLER SQL:
--    - Fewer joins needed (many FKs already in v2)
--    - More optimized for BI reporting
--
-- ============================================================================
-- USAGE NOTES
-- ============================================================================
-- 
-- GRANULARITY:
--   Still INVOICE × PERIOD level (one row per invoice per deferral period)
--   But now with ACCURATE period-level amounts!
--
-- COMPARISON WITH V1:
--   V1 used: M_INVOICE_MRR (same for all periods of an invoice)
--   V2 uses: M_AMOUNT_EUR (varies by period - MUCH MORE ACCURATE!)
--
-- COMPARISON STRATEGY:
--   Same as V1, but amounts should match silver much better now!
--
--   -- DWH view (now has accurate period amounts)
...
--
-- ============================================================================
-- END OF VIEW DEFINITION (VERSION 2.0)
-- ============================================================================
